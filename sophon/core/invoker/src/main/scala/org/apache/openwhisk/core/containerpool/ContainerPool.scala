...
import org.apache.openwhisk.core.connector.{MessageFeed, ReconcileMessage}
...
class ContainerPool(childFactory: ActorRefFactory => ActorRef,
  ...
  def logContainerStart(r: Run, containerState: String, activeActivations: Int, container: Option[Container]): Unit = {
    ...
    r.msg.transid.mark(
      ...
      s"$timestamp containerStart containerState: $containerState container: $container activations: $activeActivations of max $maxConcurrent action: $actionName namespace: $namespaceName activationId: $activationId",
      ...
      if (runBuffer.isEmpty || isResentFromBuffer) {
        ...
        val createdContainer =
          ...
          if (hasPoolSpaceFor(busyPool, r.action.limits.memory.megabytes.MB)) {
            ...
            ContainerPool
              .schedule(r.action, r.msg.user.namespace.name, freePool, r.msg.container)
        ...
        createdContainer match {
          case Some(((actor, data), containerState)) =>
            (r.msg.container, data) match {
              case ("", WarmedData(c, _, _, _, _, _)) => // cold -> warm, a warm container unknown to scheduler was found, notifying scheduler
                logging.info(this, s"${System.currentTimeMillis} case 1 cold -> warm conflict: ${c.id}")
                ack(r.msg.rootControllerIndex, ReconcileMessage(r.msg.transid, r.msg.activationId, invoker, r.msg.container, c.toString()))
              case ("", cns: ContainerNotStarted) => // normal cold start
                logging.info(this, s"${System.currentTimeMillis} case 2 cold: ${cns.getContainer}")
              case ("", cd: ContainerData) => // unusual
                logging.info(this, s"${System.currentTimeMillis} case 3 cold -> containerdata: ${cd.getContainer}")
              case (sched, WarmedData(c, _, _, _, _, _)) if sched == c.id.toString => // normal warm start
                logging.info(this, s"${System.currentTimeMillis} case 4 warm: ${sched}")
              case (sched, WarmedData(c, _, _, _, _, _)) if sched != c.id.toString => // unmatched warm start, notifying scheduler
                logging.info(this, s"${System.currentTimeMillis} case 5 warm: ${sched} -> ${c.id}")
                ack(r.msg.rootControllerIndex, ReconcileMessage(r.msg.transid, r.msg.activationId, invoker, r.msg.container, c.toString()))
              case (sched, cd: ContainerData) => // warm -> cold, a warm container was gone, notifying scheduler
                logging.info(this, s"${System.currentTimeMillis} case 6 warm -> containerdata conflict: ${sched} -> ${cd.getContainer}")
                ack(r.msg.rootControllerIndex, ReconcileMessage(r.msg.transid, r.msg.activationId, invoker, r.msg.container, ""))
              case _ => logging.info(this, s"${System.currentTimeMillis} case 7 impossible")
            }
            //increment active count before storing in pool map
            val newData = data.nextRun(r)
            val container = newData.getContainer
            r.started = newData.lastUsed
...
object ContainerPool {
  ...
  protected[containerpool] def schedule[A](action: ExecutableWhiskAction,
                                           ...
                                           containerID: String): Option[(A, ContainerData)] = {

    val warmed = idles.filter {
      case (_, c@WarmedData(_, `invocationNamespace`, `action`, _, _, _)) if c.hasCapacity() => true
      case _ => false
    }
    if (warmed.nonEmpty) {
      val matched = warmed.find {
        case (_, WarmedData(c, _, _, _, _, _)) if c.id.toString == containerID => true
        case _ => false
      }
      if (matched.nonEmpty) {
        return matched
      } else {
        return Some(warmed.minBy(_._2.lastUsed))
      }
    }

    val warming = idles.filter {
      case (_, c@WarmingData(_, `invocationNamespace`, `action`, _, _)) if c.hasCapacity() => true
      case _ => false
    }
    if (warming.nonEmpty) {
      return Some(warming.minBy(_._2.lastUsed))
    }

    val warmingCold = idles.filter {
          case (_, c@WarmingColdData(`invocationNamespace`, `action`, _, _)) if c.hasCapacity() => true
          case _ => false
    }
    if (warmingCold.nonEmpty) {
      return Some(warmingCold.minBy(_._2.lastUsed))
    }
    None
  }
  ...
  def props(factory: ActorRefFactory => ActorRef,
            ...
            ack: MessagingActiveAck,
            invoker: InvokerInstanceId) =
    Props(new ContainerPool(factory, feed, prewarmConfig, poolConfig, ack, invoker))
}
