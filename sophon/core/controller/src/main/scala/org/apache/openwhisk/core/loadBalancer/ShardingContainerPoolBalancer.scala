...
class ShardingContainerPoolBalancer(
  ...
  override def publish(action: ExecutableWhiskActionMetaData, msg: ActivationMessage)(
    ...
    val chosen = if (invokersToUse.nonEmpty) {
      ...
      val invoker: Option[(InvokerInstanceId, Boolean, String)] = this.synchronized(ShardingContainerPoolBalancer.schedule(
  ...
  override protected def reconcileState(aid: ActivationId, sched: String, used: String): Unit = {
    activationSlots.get(aid) match {
      case Some(activation) =>
        schedulingState.invokerSlots
          .lift(activation.invokerName.toInt)
          .foreach(_.reconcileState(sched, used, activation.fullyQualifiedEntityName, activation.memoryLimit.toMB.toInt))
      case None =>
    }
  }
...
object ShardingContainerPoolBalancer extends LoadBalancerProvider {
  ...
  def schedule(maxConcurrent: Int,
               ...
               dispatched: IndexedSeq[ContainerSemaphore[FullyQualifiedEntityName]],
               slots: Int,
               index: Int,
               step: Int,
               stepsDone: Int,
               lbConfig: ShardingContainerPoolBalancerConfig)(implicit logging: Logging, transId: TransactionId): Option[(InvokerInstanceId, Boolean, String)] = {
    ...
    val res = chooseOptimal(maxConcurrent, fqn, invokers, dispatched, slots, index, step, stepsDone, None, PositiveInfinity, 0, params)
    res match {
      case None =>
        logging.warn(this, s"${System.currentTimeMillis()} system overloaded, returning an error")
        None
      case Some(invoker) =>
        dispatched(invoker._1.toInt).acquire(fqn, slots, invoker._3)
        Some(invoker)
    }
  }
  ...
  def chooseOptimal(
    ...
    logging.info(this, s"${System.currentTimeMillis} chooseOptimal currentmin: ${minCost}")
    ...
    if (numInvokers > 0) {
      ...
      logging.info(this, s"${System.currentTimeMillis} step ${stepsDone}, cost ${cost}")
      ...
      if (stepsDone >= numInvokers || newMinCost <= (stepsDone+1)*params._2/numInvokers) { // traversed all invokers or current optimal cost must be lower than next invoker
        if (newMinCost < params._4) {
          ...
          chooseOptimal(maxConcurrent, fqn, invokers, dispatched, slots, newIndex, step, stepsDone + 1, newOptimal, newMinCost, newDistance, params)
      }
    } else {
      None
    }
...
case class ShardingContainerPoolBalancerState(
  ...
  protected[loadBalancer] var _invokerSlots: IndexedSeq[ContainerSemaphore[FullyQualifiedEntityName]] =
    IndexedSeq.empty[ContainerSemaphore[FullyQualifiedEntityName]],
  ...
  def invokerSlots: IndexedSeq[ContainerSemaphore[FullyQualifiedEntityName]] = _invokerSlots
    ...
    val logDetail = if (oldSize != newSize) {
      ...
      if (oldSize < newSize) {
        ...
        _invokerSlots = _invokerSlots ++ onlyNewInvokers.map { invoker =>
          new ContainerSemaphore[FullyQualifiedEntityName](getInvokerSlot(invoker.id.userMemory).toMB.toInt, logging)
        }
...
class WarmContainer[T](used: Instant, memoryLimit: Int, actionid: T, decayLambda: Float) {
  def lastUsed = used
  def memory = memoryLimit
  def action = actionid
  def lambda = decayLambda
  override def toString: String = s"${lastUsed}, ${memory}MB, ${action}"
}

class StartedContainer[T](memory: Int, acitionid: T, tid: Int)

/**
 * Updated semaphore considering warm container. Support concurrency = 1 only.
 */
class ContainerSemaphore[T](memoryPermits: Int, logging: Logging) extends ForcibleSemaphore(memoryPermits) {

  private val pausedContainers = mutable.Map.empty[String, WarmContainer[T]]
  private val inFlightContainers = mutable.Map.empty[String, WarmContainer[T]]
   ...
   */
  final def acquire(actionid: T, slots: Int, container: String): String = {
    if (container != "") {
      pausedContainers.get(container) match {
        case Some(found) =>
          pausedContainers.remove(container)
          inFlightContainers(container) = found
          super.tryAcquire(slots)
          container
        case None =>
          logging.error(this, s"acquire non-exist container ${container}")
          super.tryAcquire(slots)
          ""
      }
    }
   ...
   */
  final def tryAcquire(actionid: T, maxConcurrent: Int, memoryPermits: Int, params:(Double,Double,Double,Double)): (String, Float) = {
    // params: lambda, distanceWeight, evictionWeight, acceptThreshold
    val binaryCost = { (container: WarmContainer[T]) =>
      if (System.currentTimeMillis() < container.lastUsed.toEpochMilli + protectTimeInMs) PositiveInfinity else 0F
    }

    val expCost = { container: WarmContainer[T] =>
...
//      case FullyQualifiedEntityName(_, name, _, _) if name.toString.startsWith("burst_parallel") => tryAcquireNormal(actionid, maxConcurrent, memoryPermits, expCost)
  ...
  final def tryAcquireBP(actionid: T, maxConcurrent: Int, memoryPermits: Int, costFunction: WarmContainer[T] => Float): (String, Float) = {
    ("", PositiveInfinity)
  }

  final def tryAcquireNormal(actionid: T, maxConcurrent: Int, memoryPermits: Int, costFunction: WarmContainer[T] => Float): (String, Float) = {
    // assume maxConcurrent == 1 for now
    val debug = true
    pausedContainers.synchronized {
      if (super.availablePermits < memoryPermits) {
        if (debug) logging.info(this,s"${System.currentTimeMillis} not enough memory for action ${actionid}")
        return ("", PositiveInfinity)
      }
      // clean up timeouts
      pausedContainers.retain((_, c)=>c.lastUsed.isAfter(Instant.now.minusMillis(containerTimeout)))
      if (debug) logging.info(this,s"${System.currentTimeMillis} try to acquire ${memoryPermits} MB, current containers: ${pausedContainers}")
      ...
      if (warmContainers.nonEmpty) { // should have a warm start
        if (debug) logging.info(this,s"${System.currentTimeMillis} a warm container for ${actionid}")
      ...
      if (freeMemory < memoryPermits) { // need eviction
        if (debug) logging.info(this,s"${System.currentTimeMillis} not enough free memory for action ${actionid}, may evict")
        return ("", tryEvict(pausedContainers.clone(), memoryPermits - freeMemory, costFunction, 0F))
      }
      // all other cold starts should work
      if (debug) logging.info(this,s"${System.currentTimeMillis} a cold start for action ${actionid}")
      return ("", 0F)
    }
  }
  ...
  final def evict(needEvict: Long): Unit = {
    ...
    val lru = pausedContainers.minBy(_._2.lastUsed)
    pausedContainers.remove(lru._1)
    evict(needEvict - lru._2.memory)
  }

  @tailrec
  final def tryEvict(remainingContainers: mutable.Map[String, WarmContainer[T]], needEvict: Long, costFunc: WarmContainer[T] => Float, costSoFar: Float): Float = {
    if (needEvict <= 0) return costSoFar
    val lru = remainingContainers.minBy(_._2.lastUsed)
...
//    logging.info(this,s"force acquire ${memoryPermits} MB")
//    pausedContainers.synchronized{
//      super.forceAcquire(memoryPermits)
//    }
//  }

  def releaseConcurrent(actionid: T, maxConcurrent: Int, memoryPermits: Int, schedContainer: String, usedContainer: String, lastUsed: Instant, lambda: Float): Unit = {
    val debug = false
    ...
    pausedContainers.synchronized {
      ...
      inFlightContainers.remove(usedContainer)
      pausedContainers(usedContainer) = new WarmContainer[T](lastUsed, memoryPermits, actionid, lambda)
    }
  }
  ...
  def removeContainer(container: String): Unit = {
    pausedContainers.synchronized {
      ...
      if (pausedContainers.remove(container).isEmpty) logging.warn(this, s"invoker removed container ${container} but not found in warm pool")
      if (inFlightContainers.remove(container).nonEmpty) logging.warn(this, s"invoker removed container ${container} but has pending invocations to it")
    }
  }

  def reconcileState(sched: String, used: String, actionid:T, memoryLimit: Int): Unit = {
    pausedContainers.synchronized {
      (sched, used) match {
        case ("", used) if used != "" => // found an unexpected warm container
          pausedContainers.remove(used) match {
            case Some(container) =>
              inFlightContainers(used) = container
            case None =>
              inFlightContainers(used) = new WarmContainer[T](Instant.EPOCH, memoryLimit, actionid, 0) // placeholder
          }
        case (sched, "") if sched != "" => // warm container not found and use cold instead
          inFlightContainers.remove(sched) // don't move to paused pool because sched likely already gone
        case (sched, used) => // for unmatched warm container case we wait for release message
      }
    }
  }
}
