...
abstract class CommonLoadBalancer(config: WhiskConfig,
  ...
  protected def setupActivation(msg: ActivationMessage,
                                ...
                                container: String = ""): Future[Either[ActivationId, WhiskActivation]] = {
    ...
    logging.info(this, s"${System.currentTimeMillis} totalActivationMemory $totalActivationMemory")
    ...
    activationSlots.getOrElseUpdate(
      msg.activationId, {
        val timeoutHandler = actorSystem.scheduler.scheduleOnce(completionAckTimeout) {
          processCompletion(msg.activationId, msg.transid, forced = true, isSystemError = false, isColdStart = false, container = "", invoker = instance)
        }
      ...
      case Success(acknowledegment) =>
        acknowledegment.isSlotFree.foreach { invoker =>
          processCompletion(
            ...
            isColdStart = acknowledegment.isColdStart,
            container = acknowledegment.container,
            invoker,
            lastUsed = Instant.ofEpochMilli(acknowledegment.lastUsed))
        }
        ...
        acknowledegment.removeContainer.foreach { container =>
          removeContainer(container._1, container._2)
        }

        acknowledegment.reconcile.foreach { containers =>
          reconcileState(acknowledegment.activationId, containers._1, containers._2)
        }
  ...
  protected def releaseInvoker(invoker: InvokerInstanceId, entry: ActivationEntry, schedContainer: String, usedContainer: String, lastUsed: Instant): Unit

  protected def removeContainer(invoker: InvokerInstanceId, container: String): Unit

  protected def reconcileState(aid: ActivationId, sched: String, used: String): Unit
  ...
  protected[loadBalancer] def processCompletion(aid: ActivationId,
                                                ...
                                                lastUsed: Instant = Instant.EPOCH): Unit = {
    ...
    activationSlots.remove(aid) match {
      case Some(entry) =>
        ...
        if (entry.container != "") {
          if (entry.container == container) logging.info(this, s"${System.currentTimeMillis} right container guess: $container" )(tid)
          else logging.info(this, s"${System.currentTimeMillis} wrong container guess: ${entry.container} -> $container" )(tid)
        }
        if (lastUsed == Instant.EPOCH) logging.warn(this, s"${System.currentTimeMillis} zero value for lastUsed")(tid)
        releaseInvoker(invoker, entry, entry.container, container, lastUsed)
        ...
        if (!forced) {
          ...
          logging.info(this, s"$timestamp received completion ack for '$aid' system error=$isSystemError")(tid)
