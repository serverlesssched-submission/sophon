...
abstract class AcknowledegmentMessage(private val tid: TransactionId) extends Message {
  ...
  def isColdStart: Boolean

  def container: String

  def lastUsed: Long

  def removeContainer: Option[(InvokerInstanceId, String)]

  def reconcile: Option[(String, String)]
}
...
case class CombinedCompletionAndResultMessage private (override val transid: TransactionId,
                                                       ...
                                                       override val isColdStart: Boolean,
  ...
  override def removeContainer: Option[(InvokerInstanceId, String)] = None
  override def reconcile: Option[(String, String)] = None
}
...
case class ResultMessage private (override val transid: TransactionId, response: Either[ActivationId, WhiskActivation])
  ...
  override def shrink = copy(response = response.flatMap(a => Left(a.activationId)))
  override def toString = activationId.asString
  override def isColdStart = false
  override def container = ""
  override def lastUsed = Instant.EPOCH.toEpochMilli
  override def removeContainer: Option[(InvokerInstanceId, String)] = None
  override def reconcile: Option[(String, String)] = None
}

case class RemoveMessage private (override val transid: TransactionId,
                                  override val activationId: ActivationId,
                                  invoker: InvokerInstanceId,
                                  container: String)
  extends AcknowledegmentMessage(transid) {
  override def messageType = "remove"
  override def isSlotFree = None
  override def result = None
  override def isSystemError = None
  override def toJson = RemoveMessage.serdes.write(this)
  override def shrink = this
  override def isColdStart = false
  override def lastUsed = 0L
  override def removeContainer = Some((invoker, container))
  override def reconcile: Option[(String, String)] = None
}

case class ReconcileMessage private (override val transid: TransactionId,
                                  override val activationId: ActivationId,
                                  invoker: InvokerInstanceId,
                                  sched: String,
                                  used: String)
  extends AcknowledegmentMessage(transid) {
  override def messageType = "reconcile"
  override def isSlotFree = None
  override def result = None
  override def isSystemError = None
  override def toJson = ReconcileMessage.serdes.write(this)
  override def shrink = this
  override def isColdStart = false
  override def container = ""
  override def lastUsed = 0L
  override def removeContainer = None
  override def reconcile: Option[(String, String)] = Some(sched, used)
}
...
object CombinedCompletionAndResultMessage extends DefaultJsonProtocol {
  ...
  private def apply(transid: TransactionId,
                    ...
                    container: String,
                    invoker: InvokerInstanceId,
                    lastUsed: Long): CombinedCompletionAndResultMessage =
    new CombinedCompletionAndResultMessage(transid, activation, isSystemError, isColdStart, container, invoker, Instant.EPOCH.toEpochMilli)
  ...
  def apply(transid: TransactionId,
            ...
            invoker: InvokerInstanceId,
            lastUsed: Long): CombinedCompletionAndResultMessage =
    new CombinedCompletionAndResultMessage(transid, Right(activation), Some(activation.response.isWhiskError), isColdStart, container, invoker, Instant.EPOCH.toEpochMilli)

  implicit private val eitherSerdes = AcknowledegmentMessage.eitherResponse
  implicit val serdes = jsonFormat7(
    CombinedCompletionAndResultMessage
      .apply(_: TransactionId, _: Either[ActivationId, WhiskActivation], _: Option[Boolean], _: Boolean, _:String, _: InvokerInstanceId, _:Long))
}
...
object CompletionMessage extends DefaultJsonProtocol {
  ...
  private def apply(transid: TransactionId,
    ...
    new CompletionMessage(transid, activation.activationId, Some(activation.response.isWhiskError), isColdStart, container, invoker, lastUsed)

  def apply(transid: TransactionId, activation: WhiskActivation, isColdStart: Boolean, container: String, invoker: InvokerInstanceId, lastUsed: Long): CompletionMessage = {
    new CompletionMessage(transid, activation.activationId, Some(activation.response.isWhiskError), isColdStart, container, invoker, lastUsed)
  }

  implicit val serdes = jsonFormat7(
    CompletionMessage.apply(_: TransactionId, _: ActivationId, _: Option[Boolean], _: Boolean, _:String, _: InvokerInstanceId, _: Long))
}
...
object RemoveMessage extends DefaultJsonProtocol {
  // this constructor is restricted to ensure the message is always created with certain invariants
//  private def apply(transid: TransactionId, activationId: ActivationId, invoker: InvokerInstanceId, container: String): RemoveMessage =
//    new RemoveMessage(transid, activationId, invoker, container)

  def apply(transid: TransactionId, activationId: ActivationId, invoker: InvokerInstanceId, container: String): RemoveMessage =
    new RemoveMessage(transid, activationId, invoker, container)

  implicit val serdes = jsonFormat4(RemoveMessage.apply(_: TransactionId, _: ActivationId, _: InvokerInstanceId, _: String))
}

object ReconcileMessage extends DefaultJsonProtocol {
  // this constructor is restricted to ensure the message is always created with certain invariants

  def apply(transid: TransactionId, activationId: ActivationId, invoker: InvokerInstanceId, sched: String, used: String): ReconcileMessage =
    new ReconcileMessage(transid, activationId, invoker, sched, used)

  implicit val serdes = jsonFormat5(ReconcileMessage.apply(_: TransactionId, _: ActivationId, _: InvokerInstanceId, _: String, _: String))
}
...
object AcknowledegmentMessage extends DefaultJsonProtocol {
  ...
  implicit val serdes = new RootJsonFormat[AcknowledegmentMessage] {
    ...
    override def read(json: JsValue): AcknowledegmentMessage = {
      ...
      val completion = fields.contains("invoker") && fields.contains("isColdStart")
      val result = fields.contains("response")
      val reconcile = fields.contains("sched")
      if (completion && result) {
        ...
        json.convertTo[ReconcileMessage]
      } else {
        json.convertTo[RemoveMessage]
      }
    }
  }
}
