...
class MessagingActiveAck(producer: MessageProducer, instance: InstanceId, eventSender: Option[EventSender])(
  ...
  override def apply(controllerInstanceId: ControllerInstanceId, msg: AcknowledegmentMessage): Unit = {
    producer.send(topic = "completed" + controllerInstanceId.asString, msg)
  }
