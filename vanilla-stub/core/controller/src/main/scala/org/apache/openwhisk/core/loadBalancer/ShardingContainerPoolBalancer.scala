        ...
        if (stepsDone == numInvokers + 1) {
          if (lbConfig.invokerQueue) {
            ...
            if (healthyInvokers.nonEmpty) {
              ...
              val timestamp: Long = System.currentTimeMillis
              logging.warn(this, s"$timestamp system is overloaded. Chose invoker${random.toInt} by random assignment.")
              Some(random, true)
            } else {
              None
            }
          } else {
            logging.warn(this, s"${System.currentTimeMillis()} system is overloaded. return an error")
            None
          }
        } else {
          val newIndex = (index + step) % numInvokers
          schedule(maxConcurrent, fqn, invokers, dispatched, slots, newIndex, step, lbConfig, stepsDone + 1)
        }
      }
    } else {
      None
