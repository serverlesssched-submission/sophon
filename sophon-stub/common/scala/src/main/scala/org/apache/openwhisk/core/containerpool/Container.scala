...
trait Container {
  ...
  def run(parameters: JsObject,
          ...
          name: String)(implicit transid: TransactionId): Future[(Interval, ActivationResponse)] = {
    ...
    val start =
      transid.started(
        ...
        s"$timestamp sending arguments to $actionName at $id $addr",
        logLevel = InfoLevel)
      ...
      .andThen { // never fails
        case Success(r: RunResult) =>
          ...
          transid.finished(
            this,
            start.copy(start = r.interval.start),
            s"$timestamp running result: ${r.toBriefString}",
            endTime = r.interval.end,
            logLevel = InfoLevel)
        case Failure(t) =>
