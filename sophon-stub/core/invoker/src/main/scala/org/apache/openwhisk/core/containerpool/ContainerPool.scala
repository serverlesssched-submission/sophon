        ...
        createdContainer match {
          case Some(((actor, data), containerState)) =>
            (r.msg.container, data) match {
              case ("", WarmedData(c, _, _, _, _, _)) => logging.info(this, s"${System.currentTimeMillis} case 1 cold -> warm: ${c.id}")
              case ("", cns: ContainerNotStarted) => logging.info(this, s"${System.currentTimeMillis} case 2 cold: ${cns.getContainer}")
              case ("", cd: ContainerData) => logging.info(this, s"${System.currentTimeMillis} case 3 cold -> containerdata: ${cd.getContainer}")
              case (sched, WarmedData(c, _, _, _, _, _)) if sched == c.id.toString => logging.info(this, s"${System.currentTimeMillis} case 4 warm: ${sched} -> ${c.id}")
              case (sched, WarmedData(c, _, _, _, _, _)) if sched != c.id.toString => logging.info(this, s"${System.currentTimeMillis} case 5 warm: ${sched} -> ${c.id}")
              case (sched, cd: ContainerData) => logging.info(this, s"${System.currentTimeMillis} case 6 warm -> containerdata: ${sched} -> ${cd.getContainer}")
