...
import org.apache.openwhisk.core.connector.{ActivationMessage, CombinedCompletionAndResultMessage, CompletionMessage, RemoveMessage, ResultMessage}
...
case class Run(action: ExecutableWhiskAction, msg: ActivationMessage, retryLogDeadline: Option[Deadline] = None, var started: Instant = Instant.EPOCH)
case object Remove
...
class ContainerProxy(factory: (TransactionId,
  ...
  var rootController: Option[ControllerInstanceId] = None
  ...
  when(Uninitialized) {
    ...
    case Event(job: Run, _) =>
      ...
      rootController = Some(job.msg.rootControllerIndex)
      ...
      container
        .andThen {
          case Success(container) =>
            ...
            containerName = Some(container.toString)
          ...
          case Failure(t) =>
            ...
            sendActiveAck(
              ...
              CombinedCompletionAndResultMessage(transid, activation, true, "", instance, Instant.EPOCH.toEpochMilli))
  ...
  when(Starting) {
    ...
    case Event(completed: PreWarmCompleted, _) =>
      containerName = Some(completed.data.toString)
  ...
  def destroyContainer(newData: ContainerStarted) = {
    (rootController, containerName) match {
      case (Some(root), Some(cName)) =>
        logging.info(this, s"destroyContainer(${cName})")
        sendActiveAck(root, RemoveMessage(TransactionId(""), ActivationId("00000000000000000000000000000000"), instance, cName))
      case _ => logging.error(this, s"cannot send RemoveMessage for rootController=$rootController, containerName=$containerName")
    }
      ...
      .flatMap { initInterval =>
        ...
        container
          .run(
            ...
            job.action.limits.concurrency.maxConcurrent,
            reschedule,
            job.action.name.toString)(job.msg.transid)
          .map {
    ...
    val sendResult = if (job.msg.blocking) {
      activation.map { result =>
        val msg =
          ...
          else CombinedCompletionAndResultMessage(tid, result, false, container.toString(), instance, Instant.EPOCH.toEpochMilli)
      ...
      if (splitAckMessagesPendingLogCollection) Future.successful(())
        ...
        activation.map { result =>
          val msg = CompletionMessage(tid, result, false, container.toString(), instance, lastUsed = job.started.toEpochMilli)
          sendActiveAck(tid, result, job.msg.blocking, job.msg.rootControllerIndex, job.msg.user.namespace.uuid, msg)
        }
    }
          ...
          sendResult.onComplete(
            _ =>
              sendActiveAck(
                ...
                job.msg.rootControllerIndex,
                job.msg.user.namespace.uuid,
                CompletionMessage(tid, activation, false, container.toString(), instance, lastUsed = job.started.toEpochMilli)))
        }
