...
object DockerContainer {
  ...
  private val nextPort = new AtomicInteger(7001)
  /**
  ...
  def create(transid: TransactionId,
    ...
    val port = if (transid.meta.id != TransactionId.invokerHealth.meta.id) nextPort.getAndAdd(1) else 0
    if (port >= 32768) {
      log.error(this, s"nextPort reaches 32768!")
      throw new Exception("nextPort reaches 32768")
    }

    val netSuffix =
      if (transid.meta.id == TransactionId.invokerHealth.meta.id) {
        ""
      } else {
        port.toString
      }
    ...
    val environmentArgs =
      if (transid.meta.id == TransactionId.invokerHealth.meta.id) {
        ...
        val merged = environment ++ Map("FLASK_PROXY_PORT" -> port.toString)
        merged.flatMap {
          case (key, value) => Seq("-e", s"$key=$value")
        }
      }
    ...
    val network2 =
      if (transid.meta.id == TransactionId.invokerHealth.meta.id) {
        log.info(this, s"transid: ${transid.meta.id} equals to ${TransactionId.invokerHealth.meta.id}")
        network
      } else {
        log.info(this, s"transid: ${transid.meta.id} not equals to ${TransactionId.invokerHealth.meta.id}")
        "host"
      }
    ...
    for {
      ...
      ip <- docker.inspectIPAddress(id, network2+netSuffix).recoverWith {
        // remove the container immediately if inspect failed as
        // we cannot recover that case automatically
        case e =>
          log.warn(this, s"inspectIPAddress throws: ${e.getLocalizedMessage}")
          docker.rm(id)
          Future.failed(WhiskContainerStartupError(Messages.resourceProvisionError))
      }
